		...
		for jDep in jobDeps:
			if len(jDep) == 2:
				...
				if "localmode" in jobData and jobData["localmode"]:
					depConf = os.path.join(os.path.join(localSlavePath, "Jobs", depName, "PandoraJob.ini"))
					if not os.path.exists(depConf):
						sys.exit("ERROR - dependent JobIni does not exist %s" % depConf)
					else:
						...
						if depConfig.has_option("information", "outputpath") and depConfig.get("information", "outputpath") != "":
							...
							sys.exit("ERROR - dependent JobIni has no outputpath setting exist %s" % depConf)
				else:
