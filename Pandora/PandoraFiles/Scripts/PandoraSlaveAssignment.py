...
class PandoraSlaveAssignment(QDialog, PandoraSlaveAssignment_ui.Ui_dlg_SlaveAssignment):
	...
	def getSlaves(self):
		...
		configPath = os.getenv('LocalAppdata') + "\\Pandora\\Config\\Pandora.ini"

		if not os.path.exists(configPath):
			QMessageBox.warning(self,"Warning", "Pandora config does not exist")
			return False

		pconfig = ConfigParser()
		pconfig.read(configPath)
		...
		if self.localmode:
			if pconfig.has_option("globals", "rootpath"):
				rootPath = pconfig.get("globals", "rootpath")
				sPath = os.path.join(rootPath, "PandoraFarm", "Workstations", "WS_" + socket.gethostname())
		else:
			if pconfig.has_option("submissions", "submissionpath"):
				sPath = pconfig.get('submissions', "submissionpath")
