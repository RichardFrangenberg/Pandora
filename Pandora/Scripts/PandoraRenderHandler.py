...
class RenderHandler(QMainWindow, RenderHandler_ui.Ui_mw_RenderHandler):
		...
		try:
			...
			if self.localMode:
				if ucData["rootPath"] is not None:
					...
					self.sourceDir = os.path.join(rootPath, "Workstations", "WS_" + socket.gethostname(), "")
			...
			self.core.callback(name="onRenderHandlerOpen", types=["curApp", "custom"], args=[self])
	...
	def loadLayout(self, preUpdate=True):
		...
		self.core.callback(name="onPrismRenderHandlerOpen", types=["curApp", "custom"], args=[self])
	...
	def closeEvent(self, event):
		...
		self.core.callback(name="onRenderHandlerClose", types=["curApp", "custom"], args=[self])
			...
			for i in os.listdir(slaveDir):
				try:
					...
					if i.startswith("slaveLog_") and i.endswith(".txt") and os.path.isfile(slaveLogPath):
						...
						slaveWarningsPath = slaveLogPath.replace("slaveLog_", "slaveWarnings_")[:-3] + "json"
	...
	def rclList(self, listType, pos, twItem=None):
		...
		if self.tw_jobs.rowCount() > 0 and len(selJobs) > 0:
			...
			if os.path.exists(jobSettings):
				jobSettings = lambda: self.core.openFile(self.tw_jobs.item(self.tw_jobs.currentRow(),9).text())
			else:
				jobSettings = lambda: self.core.openFile("")
			jobFolder = lambda: self.core.openFile(os.path.dirname(self.tw_jobs.item(self.tw_jobs.currentRow(),9).text()))
		...
		if self.tw_slaves.rowCount() > 0 and listType in ["s", "sl", "ss", "scmd", "sw"]:
			...
			if os.path.exists(slaveLog):
				slaveLog = lambda: self.core.openFile(self.tw_slaves.item(self.tw_slaves.currentRow(), 7).text())
			else:
				slaveLog = lambda: self.core.openFile("")
			slaveSettings = self.tw_slaves.item(self.tw_slaves.currentRow(), 7).text().replace("slaveLog_", "slaveSettings_")[:-3] + "json"
			if os.path.exists(slaveSettings):
				slaveSettings = lambda: self.core.openFile(self.tw_slaves.item(self.tw_slaves.currentRow(), 7).text().replace("slaveLog_", "slaveSettings_")[:-3] + "json")
			else:
				slaveSettings = lambda: self.core.openFile("")
			slaveFolder = lambda: self.core.openFile(os.path.dirname(self.tw_slaves.item(self.tw_slaves.currentRow(), 7).text()))
