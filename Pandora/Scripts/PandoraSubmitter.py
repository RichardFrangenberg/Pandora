...
import sys, os, time, traceback, shutil, socket
...
If set to False, the renderings can be found locally on the renderslave, which rendered the job.""")
	...
	def saveSettings(self):
		...
		cData.append(["lastUsedSettings", "resolutionOverride", self.chb_resOverride.isChecked()])
		...
		cData.append(["lastUsedSettings", "dependentFiles", self.chb_dependencies.isChecked()])
		cData.append(["lastUsedSettings", "uploadOutput", self.chb_uploadOutput.isChecked()])
	...
	def startSubmission(self):
		...
		outputName = os.path.join(os.path.dirname(self.e_outputpath.text()), "%s%s.exr" % (os.path.splitext(os.path.basename(self.e_outputpath.text()))[0], self.core.appPlugin.frameString))
