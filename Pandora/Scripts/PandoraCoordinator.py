...
try:
	import _winreg
	pndPath = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "PythonLibs", "Renderslave")
...
class PandoraCoordinator():
	...
	def __init__(self):
		try:
			...
			pandoraConfig = os.path.join(os.environ["userprofile"], "Documents", "Pandora", "Pandora.json")
			self.repPath = ""
			self.localMode = True
			...
			if not curPath.replace("\\", "/").endswith("/Scripts/PandoraCoordinator"):
				...
				tslScript = os.path.join(os.path.dirname(self.coordBasePath), "PandoraSlaves", "PandoraSlave.py")
	...
	def writeWarning(self, text, level=2):
		...
		self.setConfig(configPath=self.coordWarningsConf, confData=warningConfig, suppressError=True)
	...
	@err_decorator
	def getConfig(self, cat=None, param=None, data=None, configPath=None, getOptions=False, getItems=False, getConf=False, suppressError=False):
		try:
	...
	@err_decorator
	def setConfig(self, cat=None, param=None, val=None, data=None, configPath=None, delete=False, confData=None, suppressError=True):
		try:
			if configPath is None:
				configPath = self.coordConf
	...
	def restoreConfig(self, configPath):
		...
		try:
			...
			warnStr = "Could not remove corrupt config in order to restore a backup config:\n\n%s" % configPath
			self.writeWarning(warnStr, 2)
		...
		if command is not None and type(command) == list:
			for i in range(1):
				if command[0] == "taskUpdate":
					...
					if settingType == "Job":
						settingsPath = os.path.join(self.jobPath, parentName, "PandoraJob.json")
						section = "jobglobals"
					...
					self.setConfig(section, settingName, settingVal, configPath=settingsPath)
					...
					for m in os.listdir(os.path.join(self.slPath, "Workstations")):
						...
						logFile = os.path.join(self.slPath, "Workstations", m, "Logs", "Jobs", "PandoraJob.json")
					...
					taskData = self.getConfig("jobtasks", "task%04d" % taskNum, configPath=jobConf)
					if taskData is None:
						self.writeLog("Job %s has no task %s (%s)" % (jobName, taskNum, origin), 2)
						continue

					if taskData[2] in ["rendering", "assigned"]:
						self.sendCommand(taskData[3], ["cancelTask", jobCode, "task%04d" % taskNum])
					...
					if warnType == "Coordinator":
						warningConfig = self.getConfig(configPath=self.coordWarningsConf, getConf=True)
						...
						self.setConfig(configPath=self.coordWarningsConf, confData=warningConfig)
					...
					if warnType == "Coordinator":
						...
						self.setConfig(confData=warnConfig, configPath=self.coordWarningsConf)
			...
			try:
				...
				for k in os.listdir(jobDir):
					...
					if not os.path.exists(jobConf):
						self.writeWarning("Job config does not exist for job: %s (%s)" % (jobCode, wsName), 2)
						continue
					...
					if cData["projectAssets"] is not None:
						wspaFolder = os.path.join(jobDir, "ProjectAssets", cData["projectName"])
						...
						for m in cData["projectAssets"]:
							...
							if os.path.exists(aPath) and int(os.path.getmtime(aPath)) == int(m[1]):
								try:
									...
									self.writeWarning("Could not copy file to ProjectAssets: %s %s %s %s" % (wsName, cData["projectName"], jobName, m), 2)
							else:
	...
	def checkSlaves(self):
		...
		for i in os.listdir(os.path.join(self.slPath, "Slaves")):
			try:
				...
				masterScriptPath = os.path.join(self.slPath, "Scripts", "PandoraSlaves", "PandoraSlave.py")
	...
	def checkRenderingTasks(self):
		...
		for i in self.renderingTasks:
			...
			rSlave = taskData[3]
			slaveSettings = os.path.join(self.slPath, "Slaves", "S_%s" % rSlave, "slaveSettings_%s.json" % rSlave)
	...
	def getAvailableSlaves(self):
		...
		for i in self.activeSlaves:
			try:
				...
				slaveSettings = os.path.join(self.slPath, "Slaves", "S_%s" % i, "slaveSettings_%s.json" % i)
				slaveStatus = self.getConfig("slaveinfo", "status", configPath=slaveSettings)

				if slaveStatus is None:
					self.writeLog("Slavesettings from %s don't have an option \"status\" in \"slaveinfo\"" % i, 2)
					continue
	...
	def assignJobs(self):
		...
		for jobDir in self.jobDirs:
			...
			if cData["jobDependecies"] is not None:
				...
				for jDep in jobDeps:
					if len(jDep) == 2:
						...
						if "information" in depConfig and "jobName" in depConfig["information"]:
			...
			if cData["listSlaves"] is not None:
				...
				for i in self.availableSlaves:
					...
					if listSlaves.startswith("groups: "):
						...
						slaveSettings = os.path.join(self.slPath, "Slaves", "S_%s" % i, "slaveSettings_%s.json" % i)
						slaveGroups = self.getConfig("settings", "slaveGroup", configPath=slaveSettings)
			...
			for i in sorted(jobConfig["jobtasks"]):
				...
				slaveSettings = os.path.join(slavePath, "slaveSettings_%s.json" % assignedSlave)
	...
	def collectOutput(self, slave=None, jobCode=None):
		...
		if cData["submitWorkstation"] is not None:
			targetBase = os.path.join(self.slPath, "Workstations", "WS_" + cData["submitWorkstation"], "RenderOutput", cData["projectName"], jobCode)
		else:
			return [0,0, ""]
		...
		if "jobtasks" in jconfig["jobtasks"]:
			for k in jconfig["jobtasks"]:
				...
				targetConf = os.path.join(self.slPath, "Workstations", "WS_" + cData["submitWorkstation"], "RenderOutput", cData["projectName"], jobCode, "PandoraJob.json")
				if os.path.exists(targetConf):
					outputCount = 0
					for i in os.walk(os.path.dirname(targetConf)):
						outputCount += len(i[2])

					self.setConfig("information", "outputFileCount", outputCount, configPath=targetConf)
	...
	def notifyWorkstations(self):
		...
		for i in os.listdir(os.path.join(self.slPath, "Slaves")):
			if i.startswith("S_"):
				...
				slaveSettings = os.path.join(os.path.dirname(slaveLog), "slaveSettings_%s.json" % slaveName)
				slaveWarnings = os.path.join(os.path.dirname(slaveLog), "slaveWarnings_%s.json" % slaveName)
