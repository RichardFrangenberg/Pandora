...
import sys, os, _winreg, shutil, time, io, multiprocessing, threading, socket, subprocess, traceback, json
from functools import wraps

pandoraRoot = os.path.dirname(os.path.dirname(__file__))
pndPath = os.path.join(pandoraRoot, "PythonLibs", "Renderslave")
...
class SlaveLogic(QDialog):
	...
	def __init__(self, core):
		...
		self.core = core
		self.slaveLogicVersion = "v1.0.3.12"
		...
		self.useRestPeriod = False 			# restperiod defines a time in which the slave does not render
...
#		self.connectionTimeout = 15
		...
		self.slaveWarningsConf = os.path.join(self.slavePath, "slaveWarnings_%s.json" % socket.gethostname())
	...
	def createTrayIcon(self):
		...
		self.trayIconMenu.addAction(self.folderAction)
		self.trayIconMenu.addSeparator()
		self.settingsAction = QAction("Pandora Settings...", self.parentWidget, triggered=self.core.openSettings)
	...
	def restartLogic(self, restart=True):
		...
		if restart:
			...
			pwExe = os.path.join(pandoraRoot, "Python27", "PandoraSlave.exe")
...
#		self.writeLog("set gdrive path to: %s" % self.gdrive)
	...
	def setSlaveInfo(self):
		self.getConfSetting("status", section="slaveinfo", setval=True, value=self.slaveState)
		...
		self.getConfSetting("cpucount", section="slaveinfo", setval=True, value=multiprocessing.cpu_count())
		self.getConfSetting("slaveScriptVersion", section="slaveinfo", setval=True, value=self.slaveLogicVersion)
		...
		self.getConfSetting("ram", section="slaveinfo", setval=True, value=(totalMem / (1024**3)))
					

	# reads a slave setting from file and returns the value
	@err_decorator
	def getConfSetting(self,setting, section="settings", stype="string", setval=False, value=""):
		...
		if complement:
			...
			for i in sConfig["settings"]:
				if i not in curConfig["settings"]:
					...
					self.writeLog("complement config: %s %s" % (i, sConfig["settings"][i]), 1)
...
#			self.writeLog("DEBUG - waiting for files")
	...
	def openActiveQuestion(self):
		...
		if self.getConfSetting("preRenderWaitTime") is None:
			self.getConfSetting("preRenderWaitTime", setval=True, value=self.prerenderwaittime)
		else:
...
#			self.lastConnectionTime = time.time()
	...
	def checkRest(self):
		...
		if restData is None:
			self.getConfSetting("restPeriod", setval=True, value=[self.useRestPeriod, self.startRP, self.endRP])
		else:
			try:
	...
	def startRenderJob(self, command):
		...
		if "jobDependecies" in jobData:
			...
			for jDep in jobData["jobDependecies"]:
				if len(jDep) == 2:
					depName = jDep[0]
					if self.localMode:
						depConf = os.path.join(os.path.join(self.localSlavePath, "Jobs", depName, "PandoraJob.json"))
		...
		if showinterruptwindow and not (hasattr(self, "msg") and self.msg.isVisible()):
			self.msg = QMessageBox(QMessageBox.Information, "Pandora RenderSlave", "Press OK to interrupt the current rendering.")
		...
		self.getConfSetting("curjob", section='slaveinfo', setval=True, value="%s (%s)" % (jobName, taskName))
		...
		if dccPlugin is not None:
			result = dccPlugin.startJob(self, sceneFile=sceneFile, startFrame=taskData[0], endFrame=taskData[1], jobData=jobData)
		else:
	...
	def checkForUpdates(self):
		#logic update
		latestFile = self.slavePath + "\\Scripts\\%s\\PandoraSlave.py" % socket.gethostname()
	...
	def finishedJob(self, jobData):
		...
		if hasNewOutput and not self.localMode and "uploadOutput" in jobData and jobData["uploadOutput"]:
			for i in os.walk(basePath):
				for k in i[2]:
