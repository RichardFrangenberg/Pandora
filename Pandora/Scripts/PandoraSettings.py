...
class PandoraSettings(QDialog, PandoraSettings_ui.Ui_dlg_PandoraSettings):
	def __init__(self, core):
		...
		self.core.callback(name="onPandoraSettingsOpen", types=["curApp", "custom"], args=[self])
	...
	def connectEvents(self):
		self.e_username.textChanged.connect(lambda x: self.validate(self.e_username, x, allowChars=[" "]))
	...
	def addSlave(self, itemType):
		...
		if action == 1:
			...
			if itemName == "":
				QMessageBox.warning(self.core.messageParent, "Error", "Invalid %s name" % itemType)
				return

			itemPath = os.path.join(basePath, prefix + itemName)

			if os.path.exists(itemPath):
				QMessageBox(self.core.messageParent, "Error", "%s%s %s already exists" % (itemType[0].upper(),itemType[1:], itemName))
				return

			try:
				os.makedirs(itemPath)
			except:
				QMessageBox(self.core.messageParent, "Error", "Could not create %s %s" % (itemType, itemName))
			else:
				refresh()
	...
	def saveSettings(self):
		...
		for i in self.exOverridePlugins:
			cData.append(['dccoverrides', "%s_override" % i, self.exOverridePlugins[i]["chb"].isChecked()])
			cData.append(['dccoverrides', "%s_path" % i, self.exOverridePlugins[i]["le"].text()])
		...
		if self.gb_coordinator.isChecked() and self.chb_coordinatorStartup.isChecked():
			if not os.path.exists(coordStartup):
				cPath = os.path.join(self.core.pandoraRoot, "Tools", "PandoraCoordinator.lnk")
		...
		for i in pluginNames:
			...
			#	w_ovr.setMinimumSize(0,39)
				...
				if self.core.getPluginData(i, "canOverrideExecuteable") != False:
					l_ovr = QLabel("By default Pandora Slave tries to find the correct version of an application to render scenes.\nThe following setting let you override this behaviour by defining an explicit application for rendering.")
	...
	def refreshIntegrations(self):
		...
		if os.path.exists(self.core.installLocPath):
			installConfig = self.core.getConfig(configPath=self.core.installLocPath, getConf=True)
