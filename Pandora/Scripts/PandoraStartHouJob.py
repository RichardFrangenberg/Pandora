...
try:
		...
		try:
			...
			if "$F" in i[1]:
				...
				if countExp == 2:
					...
					endName = os.path.basename(i[1])[os.path.basename(i[1]).find("$F")+3:]

				for k in depFiles:
					if os.path.basename(k).startswith(startName) and os.path.basename(k).endswith(endName):
						fseq = True
						break
			...
			if (os.path.basename(i[1]) in depFiles and i[0] is not None) or fseq:
				try:
					...
					writeLog("Cannot relink file dependency, because the parameter is locked: %s - %s" % (i[0], os.path.basename(i[1])))

		except Exception,e:
			exc_type, exc_obj, exc_tb = sys.exc_info()
			writeLog("ERROR - PandoraStartHouJob - repath dependencies - %s - %s - %s" % (str(e), exc_type, exc_tb.tb_lineno))
		...
		for jDep in jobDeps:
			if len(jDep) == 2:
				...
				if "localMode" in jobData and jobData["localMode"]:
					depConf = os.path.join(os.path.join(localSlavePath, "Jobs", depName, "PandoraJob.json"))
					if not os.path.exists(depConf):
						...
						if "information" in depConfig and "outputPath" in depConfig["information"] and depConfig["information"]["outputPath"] != "":
							depPath = os.path.dirname(depConfig["information"]["outputPath"])
						else:
							sys.exit("ERROR - dependent job config has no outputpath setting exist %s" % depConf)
				else:
					depPath = os.path.join(os.path.join(localSlavePath, "RenderOutput", depName))
				...
				for i in os.walk(depPath):
					if len(i[2]) > 0:
						...
						if depNode.isEditable():
							...
							writeLog("Cannot relink file dependency, because the parameter is locked: %s - %s" % (depNode.parm("file"), fPath))
	...
	if "outputPath" in jobData:
		...
		if "localMode" in jobData and jobData["localMode"]:
			...
			newOutput = os.path.join(localSlavePath, "RenderOutput", jobData["jobcode"], os.path.basename(os.path.dirname(curOutput)), os.path.basename(curOutput))
	else:
	...
	if renderNode.type().name() == "ifd":
		...
		for i in range(numAovs):
			...
			passOutputName = os.path.join(os.path.dirname(os.path.dirname(newOutput)), passName, os.path.basename(newOutput).replace("beauty", passName))
...
return outPut"""
	...
	exc_type, exc_obj, exc_tb = sys.exc_info()
	writeLog("ERROR - PandoraStartHouJob - %s - %s - %s" % (str(e), exc_type, exc_tb.tb_lineno))
	sys.exit("ERROR - PandoraStartHouJob - %s - %s - %s" % (str(e), exc_type, exc_tb.tb_lineno))
