...
class Pandora_3dsMax_Functions(object):
	...
	def getSceneCameras(self, origin):
		cams = self.executeScript(origin, "for i in cameras where (superclassof i) == camera collect i")
	...
	def getExternalFiles(self, origin, isSubmitting=False):
		extFiles = self.executeScript(origin, "mapfiles=#()\n\
fn addmap mapfile =\n\
(\n\
		...
		if activePasses and elementMgr.NumRenderElements() > 0 and separateAOVs:
			...
			for i in range(elementMgr.NumRenderElements()):
				...
				try:
					...
					pass
				self.executeScript(origin, "(maxOps.GetCurRenderElementMgr()).SetRenderElementFilename %s \"%s\"" % (i, passOutputName.replace("\\","\\\\")), returnVal=False)
