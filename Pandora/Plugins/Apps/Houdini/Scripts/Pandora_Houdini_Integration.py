...
class Pandora_Houdini_Integration(object):
	...
	def writeHoudiniFiles(self, houdiniPath):
		try:
			...
			if not os.path.exists(houdiniPath):
				QMessageBox.warning(self.core.messageParent, "Pandora Installation", "Invalid Houdini path: %s.\n\nThe path has to be the Houdini preferences folder, which usually looks like this: (with your Houdini version):\n\n%s" % (houdiniPath, self.examplePath))
				return False
			...
			if os.path.exists(pyrc):
				...
				if not initString in content:
					if "#>>>PandoraStart" in content and "#<<<PandoraEnd" in content:
						content = content[:content.find("#>>>PandoraStart")] + content[content.find("#<<<PandoraEnd")+len("#<<<PandoraEnd"):] + initString
			...
			# pandoraInit
			initpath = os.path.join(os.path.dirname(pyrc), "Pandora.py")
			...
			origInitFile = os.path.join(integrationBase, "Pandora.py")
	...
	def removeIntegration(self, installPath):
		try:
			...
			initPy = os.path.join(installBase, "python2.7libs", "Pandora.py")
			initPyc = initPy + "c"
