...
class Pandora_Houdini_Functions(object):
	...
	def startup(self, origin):
		...
		if not shelfName in hou.shelves.shelves():
			...
			submitterScript = "import Pandora\n\nPandora.core.openSubmitter()"
			if hou.shelves.tool("pandora_submitter") is not None:
				hou.shelves.tool("pandora_submitter").destroy()
			hou.shelves.newTool(file_path=hou.shelves.defaultFilePath(), name = "pandora_submitter", label ="Submitter",help = "\"\"\"Open the Pandora renderjob submitter\"\"\"", script= submitterScript, icon=os.path.join(self.core.pandoraRoot, "Scripts", "UserInterfacesPandora", "pandoraSubmitter.png" ).replace("\\", "/"))

			renderHandlerScript = "import Pandora\n\nPandora.core.openRenderHandler()"
			if hou.shelves.tool("pandora_renderhandler") is not None:
				hou.shelves.tool("pandora_renderhandler").destroy()
			hou.shelves.newTool(file_path=hou.shelves.defaultFilePath(), name = "pandora_renderhandler", label ="Render-Handler",help = "\"\"\"Open the Pandora Render-Handler\"\"\"", script= renderHandlerScript, icon=os.path.join(self.core.pandoraRoot, "Scripts", "UserInterfacesPandora", "pandoraRenderHandler.png").replace("\\", "/"))

			settingsScript = "import Pandora\n\nPandora.core.openSettings()"
			if hou.shelves.tool("pandora_settings") is not None:
				hou.shelves.tool("pandora_settings").destroy()
			hou.shelves.newTool(file_path=hou.shelves.defaultFilePath(), name = "pandora_settings", label ="Settings",help = "\"\"\"Open the Pandora settings\"\"\"", script= settingsScript, icon=os.path.join(self.core.pandoraRoot, "Scripts", "UserInterfacesPandora", "pandoraSettings.png").replace("\\", "/"))
	...
	def onPrismRenderHandlerOpen(self, origin):
		...
		origin.tw_slaveWarnings.setStyleSheet(hou.qt.styleSheet().replace("QLabel", "QHeaderView"))
		origin.tw_coordWarnings.setStyleSheet(hou.qt.styleSheet().replace("QLabel", "QHeaderView"))
	...
	def preSubmitChecks(self, origin, jobData):
		renderNode = None
		try:
			if hou.node(jobData["renderNode"]).type().name() in ["ifd", "Redshift_ROP", "rop_dop", "rop_comp", "rop_geometry", "rop_alembic", "filecache", "geometry", "alembic"]:
