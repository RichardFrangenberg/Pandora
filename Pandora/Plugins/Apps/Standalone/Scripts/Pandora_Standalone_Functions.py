...
class Pandora_Standalone_Functions(object):
	...
	def createWinStartMenu(self, origin):
		...
		toolList = ["PandoraTray", "PandoraRenderHandler", "PandoraSlave", "PandoraCoordinator", "PandoraSettings"]
		...
		if os.path.exists(trayLnk):
			if os.path.exists(os.path.dirname(trayStartup)):
				...
				QMessageBox.warning(self.core.messageParent, "Pandora", "could not create PandoraTray autostart entry")
		...
		for i in toolList:
			...
			self.core.createShortcut(source, vTarget=("%s\Python27\%s.exe" % (self.core.pandoraRoot, i)), args=('"%s\Scripts\%s.py"' % (self.core.pandoraRoot, i)))
			if os.path.exists(source) and os.path.exists(os.path.dirname(target)):
				shutil.copy2(source, target)
			else:
				QMessageBox.warning(self.core.messageParent, "Pandora", "could not create %s startmenu entry" % i)