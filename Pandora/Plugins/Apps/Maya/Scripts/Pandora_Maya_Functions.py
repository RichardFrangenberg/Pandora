...
class Pandora_Maya_Functions(object):
	...
	def getExternalFiles(self, origin, isSubmitting=False):
		...
		extFiles = [self.core.fixPath(str(x)) for x in cmds.file(query=True, list=True) if self.core.fixPath(str(x)) != self.core.fixPath(prjPath) and self.core.fixPath(str(x)) != self.core.getCurrentFileName()]
		return extFiles
