...
class Pandora_Maya_Integration(object):
	...
	def writeMayaFiles(self, mayaPath):
		try:
			...
			if os.path.exists(pandoraSetup):
				...
				if not setupString in content:
					if "#>>>PandoraStart" in content and "#<<<PandoraEnd" in content:
						content = setupString + content[:content.find("#>>>PandoraStart")] + content[content.find("#<<<PandoraEnd")+len("#<<<PandoraEnd"):]
			...
			for i in icons:
				iconPath = os.path.abspath(os.path.join(__file__, os.pardir, os.pardir, os.pardir, os.pardir, os.pardir, "Scripts", "UserInterfacesPandora", i))
